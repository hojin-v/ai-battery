...
![Platform](https://img.shields.io/badge/platform-WSL%20%7C%20Windows%20HUD-blue)
...
```text
Codex ██████████ 98% │ 5h 20:47 │ 7d 95%  Claude █░░░░░░░░░ 13% │ 5h 21:10 │ 7d 59%
```
...
| Windows HUD | WSL에서 실행하는 작은 Windows floating HUD를 제공합니다. |
| npm 실행 | `npm install -g` 또는 `npx`로 실행할 수 있습니다. |

## Platform Support

| Mode | Windows native | Windows + WSL | macOS / Linux | Note |
| --- | --- | --- | --- | --- |
| `ai-battery` | 지원 | 지원 | 지원 | Node.js 18 이상이 필요합니다. |
| Claude statusLine | 지원 | 지원 | 지원 | 설치 명령은 `node <script>` 형태로 저장됩니다. |
| `ai-battery hud` | 지원 | 지원 | 미지원 | Windows 전용입니다. macOS/Linux 터미널에서는 `ai-battery --watch`를 사용하세요. |
| `ai-battery-run` | 미지원 | 지원 | 지원 | POSIX PTY와 `python3`가 필요합니다. |
...
| HUD launcher | Bash | WSL에서 Windows PowerShell HUD 실행 |
| HUD UI | PowerShell WinForms | Windows floating overlay와 tray icon |
| Data | JSONL logs, statusLine JSON | Codex/Claude 사용량 소스 |

## Source Environment

기본 CLI는 Node.js 18 이상에서 실행됩니다. `ai-battery-run`은 Python 3와 POSIX PTY가 필요합니다. HUD는 Windows PowerShell을 사용하며, Windows native와 Windows + WSL 환경을 모두 지원합니다.
...
- HUD는 Windows PowerShell 기반입니다. Windows native에서는 직접 실행하고, WSL에서는 `powershell.exe`와 `wsl.exe`를 함께 사용합니다.
